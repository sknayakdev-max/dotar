...
export default function EmployeesClient({ user, initialEmployees, initialError }: { user: User; initialEmployees: EmployeeItem[]; initialError?: string }) {
  ...
  </main></div>{dialogOpen && <InviteEmployeeDialog roles={assignableRoles} onClose={() => setDialogOpen(false)} onError={(message) => showToast(message, "error")} onCreated={(employee, password) => { setEmployees((current) => [employee, ...current]); setTemporaryPassword(password || ""); setDialogOpen(false); showToast(`${employee.name} was created successfully.`); }} />}{editing && <EditEmployeeDialog employee={editing} roles={assignableRoles} onClose={() => setEditing(null)} onError={(message) => showToast(message, "error")} onSaved={(employee) => { setEmployees((current) => current.map((item) => item.id === employee.id ? { ...item, ...employee, status: editing.status } : item)); setEditing(null); showToast(`${employee.name} was updated successfully.`); }} />}</div>;
}
