...
export default function EmployeesClient({ user, initialEmployees, initialError }: { user: User; initialEmployees: EmployeeItem[]; initialError?: string }) {
  ...
  function showToast(message: string, tone: "success" | "error" = "success", duration = 4500) {
    setToast({ message, tone });
    if (duration > 0) window.setTimeout(() => setToast(null), duration);
  }
  ...
  </main></div>{dialogOpen && <InviteEmployeeDialog roles={assignableRoles} onClose={() => setDialogOpen(false)} onError={(message) => showToast(message, "error")} onCreated={(employee, password) => { setEmployees((current) => [employee, ...current]); setDialogOpen(false); showToast(password ? `${employee.name} was created. Temporary password: ${password}` : `${employee.name} was created successfully.`, "success", password ? 0 : 4500); }} />}{editing && <EditEmployeeDialog employee={editing} roles={assignableRoles} onClose={() => setEditing(null)} onError={(message) => showToast(message, "error")} onSaved={(employee) => { setEmployees((current) => current.map((item) => item.id === employee.id ? { ...item, ...employee, status: editing.status } : item)); setEditing(null); showToast(`${employee.name} was updated successfully.`); }} />}</div>;
}
